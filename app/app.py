...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO tblPlayersImport (fldName,fldTeam,fldPosition,fldHeight,fldWeight,fldAge) VALUES (%s, %s,%s, %s,%s, %s) """
...
def api_add() -> str:
    ...
    cursor = mysql.get_db().cursor()
    inputData = (content['fldName'], content['fldTeam'], content['fldPosition'],
                 content['fldHeight'], content['fldWeight'], content['fldAge'])
    sql_insert_query = """INSERT INTO tblPlayersImport (fldName,fldTeam,fldPosition,fldHeight,fldWeight,fldAge) VALUES (%s, %s,%s, %s,%s, %s) """
    cursor.execute(sql_insert_query, inputData)
    mysql.get_db().commit()
